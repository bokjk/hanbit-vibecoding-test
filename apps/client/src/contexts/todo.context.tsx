...
import type { Todo, TodoFilter, TodoStats, Priority, CreateTodoRequest, UpdateTodoRequest } from 'types/index';
import { todoReducer, initialTodoState, todoSelectors } from './todo.reducer';
import type { TodoState, TodoAction, PendingOperation, SyncStatus, ConnectionStatus } from './todo.reducer';
import { integratedStorage, storageUtils } from '../services/integrated-storage.service';
...
interface TodoContextType {
  ...
  // 충돌 해결
  resolveConflict: (todoId: string, resolution: 'local' | 'remote') => Promise<void>;
...
export function TodoProvider({ children, enableAutoSync = true }: TodoProviderProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      syncManager.removeEventListener('connection_change', handleConnectionChange);
    };
  }, []);
  ...
  const initializeTodoContext = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to initialize todos';
  ...
  const hasConflicts = useMemo(() => todoSelectors.hasConflicts(state), [state]);
  const hasSyncErrors = useMemo(() => todoSelectors.hasSyncErrors(state), [state]);
  const pendingOperationsCount = useMemo(() => todoSelectors.getPendingOperationsCount(state), [state]);
        ...
        if (result.isOptimistic) {
          ...
          if (result.operationId) {
            dispatch({ 
              ...
              payload: { type: 'create', todoId: result.data.id, data: todoData }
            });
          }
        } else {
          // 서버 확정 응답
  ...
  const syncData = useCallback(async () => {
    ...
    try {
      ...
      if (!result.success) {
        dispatch({ type: 'ADD_SYNC_ERROR', payload: result.error || 'Sync failed' });
      } else {
        // 동기화 성공 후 데이터 다시 로드
        await loadTodos();
      }
  ...
  const resolveConflict = useCallback(async (todoId: string, resolution: 'local' | 'remote') => {
