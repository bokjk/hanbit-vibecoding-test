...
export function AuthProvider({ children, initOptions }: AuthProviderProps) {
  ...
  const eventListeners = useRef<Map<AuthEvent, Set<AuthEventListener>>>(
    new Map(),
  );
  ...
  const addEventListener = useCallback(
    (event: AuthEvent, listener: AuthEventListener) => {
      ...
      if (authService.isTokenValid()) {
        ...
        try {
          ...
          const tokenInfo = {
            ...
            tokenType: userInfoResponse.data.user.isGuest
              ? ("guest" as const)
          ...
          console.warn(
            "Failed to get user info, falling back to guest token",
            userInfoError,
          );
  ...
  const login = useCallback(async () => {
    ...
    try {
      ...
      throw new Error(
        "Full authentication is not yet implemented. Using guest mode.",
      );
    } catch (loginError) {
      const errorMessage =
  ...
  const canCreateTodos = useCallback(
    () => authSelectors.canCreateTodos(state),
    [state],
  );
  const canUpdateTodos = useCallback(
    () => authSelectors.canUpdateTodos(state),
    [state],
  );
  const canDeleteTodos = useCallback(
    () => authSelectors.canDeleteTodos(state),
    [state],
  );
  ...
  const isTokenExpiringSoon = useCallback(
    () => authSelectors.isTokenExpiringSoon(state),
    [state],
  );
  const getTimeUntilExpiration = useCallback(
    () => authSelectors.getTimeUntilExpiration(state),
    [state],
  );
  ...
  useEffect(() => {
    if (
      !authSelectors.isTokenValid(state) ||
      authSelectors.isTokenExpiringSoon(state)
    ) {
      return;
    }
