...
describe('TodoFilters Component', () => {
  ...
  it('should have responsive design classes', () => {
    ...
    // 필터 칩 컨테이너
    const filterChipsContainer = screen.getByTestId('filter-all').closest('.flex.flex-wrap.gap-2');
    expect(filterChipsContainer).toHaveClass('flex', 'flex-wrap', 'gap-2');

    // 정렬 옵션 그리드
    const sortContainer = screen.getByTestId('sort-by-select').closest('.grid.grid-cols-1.gap-3');
