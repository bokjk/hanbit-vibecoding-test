...
class OfflineStorageService {
  ...
  private cleanupOldData(): void {
    try {
      ...
      const cleanedOperations = operations.filter(op => 
        op.timestamp.getTime() > thirtyDaysAgo
      );
  ...
  getPendingOperations(): PendingOperation[] {
    return this.getItem<PendingOperation[]>(STORAGE_KEYS.PENDING_OPERATIONS) || [];
  }
   ...
   * 대기 중인 작업 추가
   */
  addPendingOperation(operation: Omit<PendingOperation, 'id' | 'timestamp' | 'retryCount'>): boolean {
    const operations = this.getPendingOperations();
  ...
  cleanupFailedOperations(maxRetries = 3): PendingOperation[] {
    ...
    const validOperations = operations.filter(op => op.retryCount < maxRetries);
  ...
  performHealthCheck(): {
    ...
    // 저장소 사용량이 많은 경우
    if (stats.storageUsed > 1024) { // 1MB 이상
