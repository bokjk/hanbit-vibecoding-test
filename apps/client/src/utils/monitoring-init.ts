...
function initializeMonitoring(): void {
  ...
  try {
    ...
    console.log('[Monitoring] Early monitoring systems initialized successfully');
...
function setupReactErrorHandler(errorReporter: ErrorReporter): void {
  ...
  if (import.meta.env.VITE_DEBUG === 'true') {
    ...
    if ((window as any).__REACT_DEVTOOLS_GLOBAL_HOOK__) {
      console.log('[Monitoring] React DevTools detected, enhanced error reporting enabled');
    }
  }
}
...
function setupCleanupHandlers(): void {
  const cleanup = () => {
    console.log('[Monitoring] Application shutting down, cleaning up monitoring systems');
