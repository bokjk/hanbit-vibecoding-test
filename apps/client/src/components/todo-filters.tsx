...
import type { FilterHelpers, SyncHelpers, TodoMetadata } from '../hooks/use-todo';
...
export function TodoFilters({ filter, onFilterChange, syncHelpers, metadata, className = '' }: TodoFiltersProps) {
  ...
  const handleFilterTypeChange = (type: FilterType) => {
    if (isLegacyFilter && typeof filter === 'object' && 'type' in filter) {
      ...
      switch (type) {
        case 'all': filterHelpers.showAll(); break;
        case 'active': filterHelpers.showActive(); break;
        case 'completed': filterHelpers.showCompleted(); break;
      }
    }
  };
  ...
  const handleSortByChange = (sortBy: SortBy) => {
    if (isLegacyFilter && typeof filter === 'object' && 'sortBy' in filter) {
      ...
      const filterHelpers = filter as FilterHelpers;
      switch (sortBy) {
        case 'createdAt': filterHelpers.sortByCreatedAt(); break;
        case 'priority': filterHelpers.sortByPriority(); break;
        case 'title': filterHelpers.sortByTitle(); break;
      }
    }
  };
  ...
  const handleSortOrderChange = (sortOrder: SortOrder) => {
    if (isLegacyFilter && typeof filter === 'object' && 'sortOrder' in filter) {
      (onFilterChange as (filter: TodoFilter) => void)({ ...filter, sortOrder });
    } else if (typeof filter === 'object' && 'sortByCreatedAt' in filter) {
      const filterHelpers = filter as FilterHelpers;
      // 정렬 순서는 메서드 파라미터로 전달
  ...
  const currentFilter = isLegacyFilter 
    ...
    : { type: 'all' as FilterType, sortBy: 'createdAt' as SortBy, sortOrder: 'desc' as SortOrder };
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3">
          <Select value={currentFilter.sortBy} onValueChange={handleSortByChange}>
          ...
          <Select value={currentFilter.sortOrder} onValueChange={handleSortOrderChange}>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              {metadata.isOnline ? (
                <>
                  <svg className="h-3 w-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192L5.636 18.364M12 12h.01M21 12c0 4.971-4.029 9-9 9s-9-4.029-9-9 4.029-9 9-9 9 4.029 9 9z" />
                  </svg>
                  오프라인 모드
                </>
              ) : (
