...
export function MonitoringDemo() {
  ...
  const simulateJSError = useCallback(() => {
    try {
      // 의도적으로 에러 발생
      throw new Error('Demo JavaScript error - this is intentional for testing');
    } catch (error) {
      console.error('Simulated error:', error);
      debugLog('JavaScript error simulated', { error: (error as Error).message });
    }
  }, [debugLog]);
  ...
  if (!isEnabled) {
    return (
      <Card className="w-full max-w-4xl mx-auto m-4">
        <CardHeader>
          <CardTitle className="text-amber-600">모니터링 시스템 비활성화</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <span className={isInitialized ? 'text-green-600' : 'text-amber-600'}>
            ...
            <div>
              ...
              <span className={import.meta.env.VITE_DEBUG === 'true' ? 'text-green-600' : 'text-gray-600'}>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {stats.performance.memoryUsage && (
                <div className="pt-2 border-t">
                  ...
                  <div className="flex justify-between">
                    <span>총 메모리:</span>
                    <span className="font-medium">
                      {(stats.performance.memoryUsage.total / 1024 / 1024).toFixed(1)}MB
                    </span>
                  </div>
                </div>
              )}
              <div className="space-y-1">
                ...
                {Object.entries(stats.performance.webVitals).map(([key, value]) => (
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span>에러 수:</span>
                <span className={`font-medium ${stats.errors.errorCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span>큐 크기:</span>
                <span className={`font-medium ${stats.analytics.queueSize > 10 ? 'text-amber-600' : 'text-green-600'}`}>
                  {stats.analytics.queueSize}개
                </span>
              </div>
              <div className="flex justify-between">
                <span>실패한 배치:</span>
                <span className={`font-medium ${stats.analytics.failedBatchesCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-xs">
                  {stats.analytics.lastFlushTime > 0 ? 
                    new Date(stats.analytics.lastFlushTime).toLocaleTimeString() : 
                    '없음'}
                </span>
              </div>
            </CardContent>
      ...
      <Card>
        ...
        <CardContent className="text-sm space-y-2">
          <p className="text-gray-600">
            이 데모는 개발 환경에서만 표시됩니다. 프로덕션에서는 모니터링이 백그라운드에서 자동으로 실행됩니다.
          </p>
          <div className="bg-gray-50 p-3 rounded text-xs space-y-1">
            <div><strong>환경변수:</strong></div>
            <div>VITE_ENABLE_MONITORING: {import.meta.env.VITE_ENABLE_MONITORING || 'undefined'}</div>
            <div>VITE_ENABLE_ERROR_REPORTING: {import.meta.env.VITE_ENABLE_ERROR_REPORTING || 'undefined'}</div>
            <div>VITE_ENABLE_ANALYTICS: {import.meta.env.VITE_ENABLE_ANALYTICS || 'undefined'}</div>
