...
import { unifiedTodoService, StorageMode } from '../services/unified-todo.service';
...
export interface UseTodoServiceReturn {
  ...
  updateTodo: (id: string, updates: Partial<Pick<Todo, 'title' | 'completed' | 'priority'>>) => Promise<Todo>;
  ...
  migrateToAPI: () => Promise<{ success: boolean; migratedCount: number; error?: string }>;
...
export function useTodoService(): UseTodoServiceReturn {
  ...
  const getTodoById = useCallback((id: string): Promise<Todo | null> => {
    return withErrorHandling(
  ...
  const createTodo = useCallback((title: string, priority: Priority = 'medium'): Promise<Todo> => {
    return withErrorHandling(
  ...
  const toggleTodo = useCallback((id: string): Promise<Todo> => {
    return withErrorHandling(
  ...
  const deleteTodo = useCallback((id: string): Promise<string> => {
    return withErrorHandling(
  ...
  const getFilteredTodos = useCallback((filter: FilterType): Promise<Todo[]> => {
    return withErrorHandling(
  ...
  const searchTodos = useCallback((query: string): Promise<Todo[]> => {
    return withErrorHandling(
  ...
  const getTodosByPriority = useCallback((priority: Priority): Promise<Todo[]> => {
    return withErrorHandling(
  ...
  const switchStorageMode = useCallback(async (mode: StorageMode): Promise<void> => {
  ...
  const migrateToAPI = useCallback((): Promise<{ success: boolean; migratedCount: number; error?: string }> => {
    return withErrorHandling(async () => {
  ...
  useEffect(() => {
    ...
    const configStorageMode = appConfig.features.apiMode ? StorageMode.API : StorageMode.LOCAL_STORAGE;
