...
import type { CreateTodoRequest, UpdateTodoRequest, TodoFilter, Priority } from 'types/index';
...
export function useTodo(): UseTodoReturn {
  ...
  const addTodo = useCallback(async (title: string, options: CreateTodoOptions = {}) => {
    const todoData: CreateTodoRequest = {
      ...
      tags: options.tags,
      userId: authContext.state.user?.id || authContext.state.guestId || 'guest',
    };

    await contextAddTodo(todoData);
  }, [contextAddTodo, authContext.state.user?.id, authContext.state.guestId]);

  const updateTodo = useCallback(async (id: string, options: UpdateTodoOptions) => {
...
export function useSingleTodo(todoId: string) {
  const { todos, updateTodo, deleteTodo, toggleTodo, canUpdate, canDelete } = useTodo();

  const todo = useMemo(() => {
    return todos.find(t => t.id === todoId) || null;
  }, [todos, todoId]);

  const updateThisTodo = useCallback(async (options: UpdateTodoOptions) => {
