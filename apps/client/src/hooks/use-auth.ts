...
export function useAuth() {
  ...
  const authState = useMemo(
    () => ({
      ...
      isTokenValid: state.tokenExpiration
        ? Date.now() < state.tokenExpiration.getTime()
        : false,
  ...
  const permissions = useMemo(
    () => ({
      ...
      canImport:
        state.permissions && "canImport" in state.permissions
          ? ((state.permissions as Record<string, unknown>)
              .canImport as boolean)
          : false,
      canShare:
        state.permissions && "canShare" in state.permissions
      ...
      isUnlimited:
        state.permissions && "unlimitedItems" in state.permissions
          ? ((state.permissions as Record<string, unknown>)
              .unlimitedItems as boolean)
          : false,
  ...
  const requireAuthenticated = useCallback(() => {
    if (!authState.isAuthenticated) {
      throw new Error(
        "Full authentication required (guest access not sufficient)",
      );
    }
  ...
  const renderIfGuest = useCallback(
    (content: React.ReactNode, fallback?: React.ReactNode) => {
