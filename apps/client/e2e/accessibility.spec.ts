...
test.describe("접근성 테스트", () => {
  ...
  test("키보드 네비게이션 테스트", async ({ page }) => {
    ...
    expect(
      ["todo-input", "search-input"].includes(focusedElement || ""),
    ).toBeTruthy();
  ...
  test("ARIA 레이블 및 역할 테스트", async ({ page }) => {
    ...
    // 체크박스의 역할 확인
    const checkboxRole =
      (await todoCheckbox.getAttribute("role")) || "checkbox";
  ...
  test("키보드 단축키 및 접근성 기능", async ({ page }) => {
    ...
    // Escape 키 동작 테스트
    const editButton = todoPage
      .getTodoItem("단축키 테스트 할 일")
    ...
    // 편집 모드가 취소되었는지 확인
    const editInput = todoPage
      .getTodoItem("단축키 테스트 할 일")
      .locator('[data-testid="edit-input"]');
    ...
    const currentFocus = await page
      ...
      .getAttribute("data-testid");
    expect(
      ["add-todo-button", "todo-checkbox"].includes(currentFocus || ""),
    ).toBeTruthy();
  });
});
