...
export const longTextData = {
  ...
  longTitle: '매우 긴 할 일 제목으로 UI에서 어떻게 처리되는지 확인하기 위한 테스트 데이터입니다. 이 텍스트는 카드 레이아웃에서 올바르게 래핑되고 표시되어야 합니다.',
  veryLongTitle: '극도로 긴 할 일 제목으로써 사용자 인터페이스의 경계 케이스를 테스트하기 위해 만들어졌습니다. 이러한 긴 텍스트가 있을 때 레이아웃이 깨지지 않고 사용자 경험이 유지되는지 확인해야 합니다. 또한 이 텍스트는 모바일 환경에서도 적절히 처리되어야 하며, 접근성 요구사항도 충족해야 합니다.'
};
