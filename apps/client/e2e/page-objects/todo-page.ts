...
export class TodoPage {
  ...
  async addTodo(title: string, priority: Priority = "medium") {
    ...
    await this.page
      .locator(`[role="option"]:has-text("${priorityText}")`)
      .click();
  ...
  async sortBy(sortBy: SortBy, sortOrder: SortOrder = "desc") {
    ...
    const sortByText =
      sortBy === "createdAt"
        ...
        : sortBy === "priority"
          ...
          : "제목";
    await this.page
      .locator(`[role="option"]:has-text("${sortByText}")`)
      .click();

    // 정렬 순서 선택
    const sortOrderSelect = this.page.locator(
    ...
    await this.page
      .locator(`[role="option"]:has-text("${sortOrderText}")`)
      .click();
  ...
  async getAllTodoTitles(): Promise<string[]> {
    const todoItems = await this.page
  ...
  async getStats(): Promise<TodoStats> {
    ...
    const completedElement = this.page.locator(
      '[data-testid="stats-completed"]',
    );
    const completionRateElement = this.page.locator(
    ...
    const [total, active, completed, completionRate] = await Promise.all([
      ...
      completionRateElement
        .textContent()
        .then((text) => parseInt((text || "0").replace("%", ""))),
    ]);
