  ...
  title: z
    ...
    .transform(title => {
      ...
      if (!validation.isSafe) {
        throw new Error(
          `제목에 위험한 패턴이 탐지되었습니다: ${validation.detectedPatterns.join(', ')}`
        );
      }
 ...
 * 요청 본문 파싱 및 검증 (보안 강화)
 */
export function parseAndValidate<T>(body: string | null, schema: z.ZodSchema<T>): T {
  if (!body) {
    throw new Error('요청 본문이 필요합니다');
  }

  // 입력 크기 검증 (DoS 공격 방지)
  if (!InputSanitizer.validateInputSize(body, 100000)) {
    // 100KB 제한
