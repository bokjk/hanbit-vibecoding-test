...
export class CSPNonce {
   ...
   * 기본 CSP 정책 (nonce 없음)
   */
  private static generateBasicCSP(
    environment: string,
    config: {
      allowUnsafeEval?: boolean;
  ...
  static injectNonceIntoHTML(html: string, nonce: string): string {
    ...
    // style 태그에 nonce 추가
    html = html.replace(/<style(?![^>]*nonce)/gi, `<style nonce="${nonce}"`);
